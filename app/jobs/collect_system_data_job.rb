...
class CollectSystemDataJob < ApplicationJob
  ...
  def get_time_to_jump
    ...
    cell_text_to_text(list_item).split(':', 2).last if list_item.present?
  end
  ...
  def update_owner_eras
    owner_era_rows.each do |row|
      era, faction = row.search('td').collect(&:text).collect(&:strip).collect(&:squish)
  ...
  def update_occupation_dates
    occupation_date_rows.each do |row|
      site_date, faction = row.search('td').collect(&:text).collect(&:strip).collect(&:squish)
      parsed_date = text_to_date(site_date)
